...
SELECT Date, SB, (Winner_Pts + Loser_Pts) AS "sum_score", 
    CASE
        WHEN (Winner_Pts + Loser_Pts) < 15 THEN "Low scoring"
        WHEN (Winner_Pts + Loser_Pts) < 25 THEN "Mid scoring"
        WHEN (Winner_Pts + Loser_Pts) < 35 THEN "Good scoring"
        ELSE "High Scoring Game"
    END  AS "game_intensity"
FROM superbowls;

SELECT MAX(Winner_Pts), MIN(Winner_Pts), AVG(Winner_Pts), MAX(Loser_Pts), MIN(Loser_Pts), AVG(Loser_Pts) FROM superbowls;

SELECT MVP, COUNT(MVP) AS "total_mvps" FROM superbowls GROUP BY MVP HAVING total_mvps >= 2;

SELECT City, COUNT(City) AS "num_host_city" FROM superbowls GROUP BY City HAVING num_host_city = 2 OR num_host_city = 10;
